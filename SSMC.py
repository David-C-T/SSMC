...
#node=ontologyNamespace[0].GO_0003677
#icMap[node]

#=====================================================PAIRWISE SS=================================================================


def get_Pairwise(c1,c2,icMap,pairwise,store,classAncestor):#mudar para term sim
    ...
    try:
        ...
        pwScore=eval("pairwise_"+ pairwise)(icMap=icMap,c1=c1,c2=c2,classAncestor=classAncestor)
        store[key]=pwScore
        return pwScore

def pairwise_Resnik1995(icMap,c1,c2,classAncestor,**args):
...
def groupwise_SimGIC(icMap,entityMatches,annotData,classAncestor,**args):
...
def groupwise_BMA(icMap,entityMatches,annotData,pairwise,**args):
    """ Computation of SimGIC groupise semantic similarity
...
def groupwise_PolGIC(icMap,entityMatches,annotData,doubleObjects=False,**args):
    """ Computation of SimGIC groupise semantic similarity
        Requires icMap dict of IC values, entityMatches dict of entity-to-entity combs, annotData dict of entity annotations, doubleObjects bool
        Returns entityMatches with computed Groupwise Similarity scores
    """
    errorList=[]
    for k,entry in entityMatches.items():
        try:
            ...
            e1_N=set(itertools.chain.from_iterable([classAncestor[x] for x in set(annotData["-"+entry[0]])]))
            #Object 2
            e2_P=set(itertools.chain.from_iterable([classAncestor[x] for x in set(annotData["+"+entry[1]])]))
            e2_N=set(itertools.chain.from_iterable([classAncestor[x] for x in set(annotData["-"+entry[1]])]))   
        except KeyError as e:
            if str(e) not in errorList:
            ...
            entityMatches[k]="NO DATA"
            continue
        #+1 for match or absence,-1 for unmatch, total is each concept or each variant
        intersect=list(e1_P.intersection(e2_P).union(e1_N.intersection(e2_N)))
        diff=list(e1_P.intersection(e2_N).union(e1_N.intersection(e2_P)))
        union=list(set.union(e1_P,e1_N,e2_P,e2_N))
        if doubleObjects:
            union*=2
        score=sum([icMap[x] for x in intersect])-sum([icMap[x] for x in diff]) 
        entityMatches[k].append(score/sum([icMap[x] for x in union]))
    return entityMatches


def groupwise_PolBMA(icMap,entityMatches,annotData,pairwise,cPos=1,cNeg=1,**args):
    """ Computation of PolBMA groupise semantic similarity
        Requires icMap dict of IC values, entityMatches dict of entity-to-entity combs, annotData dict of entity annotations, int cPos and cNeg with weights to add to positive and negative values
        Returns entityMatches with computed Groupwise Similarity scores
    """
    store={}
    errorList=[]
    for k,entry in entityMatches.items():
        try:      
            e1_P=set(annotData["+"+entry[0]])
            e1_N=set(annotData["-"+entry[0]])
            e2_P=set(annotData["+"+entry[1]])
            e2_N=set(annotData["-"+entry[1]])   
        except KeyError as e:
        ...
        orderedDict = {k:v for k,v in {"+":[e1_P,e2_P],"-":[e1_N,e2_N]}.items() if v!=[set(),set()]}#Remove polarities with no annotations
        scores=[]
        for pol in orderedDict:
            e1=orderedDict[pol][0]
            e2=orderedDict[pol][1]
            b=[get_Pairwise(c1=x,c2=y,icMap=icMap,pairwise=pairwise,store=store,classAncestor=classAncestor) for x,y in list(product(e1,e2))]
            if len(b)==0:
                score=0
            elif len(b)==1:
                ...
                score=(sim1+sim2)/2
            if pol == '+':
                scores.append(cPos*score)
            else:
                scores.append(cNeg*score)
        entityMatches[k].append(sum(scores)/len(scores))
    return entityMatches
...
def SSMC(settings,queries,annotData, classAncestor, classDescendant, classIC):
    ...
    for x in queries:
        ...
        for ssm in querrySSMs:#Add possible arguments for a SSM for universal call (spares if conditions)
            for arg in ['IC','Pairwise','Double_Objects','cPos','cNeg']:
                if arg not in ssm:
                    ssm[arg]=None
            ...
            if "Groupwise" in ssm:
                ...
                logging.info(ssm["IC"])
                b=eval("groupwise_"+ssm["Groupwise"])(entityMatches=b,pairwise=ssm["Pairwise"],doubleObjects=ssm["Double_Objects"],icMap=eval("classIC[\""+str(ssm["IC"])+"\"]"),annotData=annotData,classAncestor=classAncestor,classDescendant=classDescendant)
            else:
    ...
    #Clustering
        if "Clusters" in queries[x]:
            for ssm in queries[x]["Clusters"]["Score"].split(","): 
                ...
                targetScore="_".join([x[1] for x in list(querrySSMs[querryIDs.index(ssm)].items()) if x[1] not in [None, True, False]])     
